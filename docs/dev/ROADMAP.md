...
- [x] **Active-waypoint marker on the map** — `MSP_NAV_STATUS` (live) + blackbox/ArduPilot `active_wp_number` (replay) highlight the current target WP with a pulsing green glow on the icon (0.5 Hz, FBH included), gated on NAV_WP mode **and** mission trust (see provenance below). INAV done; ArduPilot layer + Flight-Mode-widget readout pending
