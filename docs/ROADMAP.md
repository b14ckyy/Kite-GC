...
- [~] **Conflict-alert tuning (C3)** — _next_: expose the ~5 numeric thresholds as user settings, grouped under 2 toggles (proximity + CPA), quick & dirty. Pre-recorded callout audio for engines without TTS (Linux WebKitGTK) stays a later add-on.
...
- [ ] **BUG: Terrain analysis ignores MAVLink (ArduPilot/PX4) missions** — Waypoint mode reads only the INAV `mission` store; ArduPilot/PX4 missions live in the separate `arduMission` store, so the profile is empty for them. Needs a unified active-mission source (+ Ardu alt-frame→MSL conversion) feeding the Terrain Analyzer (and ideally its edit path). Overlooked in the multi-autopilot work.
