...
- [~] Geozone editor — _INAV ≥8.0 FC geozones in the Airspace Manager. **P1 (display) + P2 (editor) + mission safety check shipped**: backend read/write+EEPROM, 2D+3D overlays (MWPTools type/action line+fill scheme, blue incl / amber excl), 5th layer toggle + zone list, map-first editing (drag handles + WP-style popup + add/delete) with sanity checks, and an altitude-aware mission↔geozone safety check (warns on NFZ cross / inclusion leave / launch-in-NFZ, red legs in 2D+3D, hints only). **P3 done**: in-flight breach toast — since INAV doesn't expose its geozone-avoidance mode (`NAV_SEND_TO`) over MSP, the GCS detects an actual breach geometrically (3D) from the live position and toasts on onset (NFZ / left inclusion). Open: global `geozone_*` behaviour settings. See `docs/active/GEOZONES.md`_
- [~] Geofence editor — _ArduPilot/PX4 FC geofence in the Airspace Manager (third safety subsystem, MAVLink `MAV_MISSION_TYPE_FENCE`). **Full editor shipped**: backend read/write (params + geometry), 2D+3D overlays (blue incl / amber excl), 6th layer toggle + Fence section, map-first editing (drag handles + WP-style popup + add/delete), global params with friendly labels/units/bounds + a vehicle-aware breach-action dropdown. Open: SITL verification. (In-flight breach: the FC's MAVLink STATUSTEXT is the primary cue → system messages; a geometric fence-breach toast fallback also ships via the shared breach monitor.) See `docs/active/GEOFENCE.md`_
- [~] Rally points editor — _ArduPilot/PX4 RTL divert points (`MAV_MISSION_TYPE_RALLY`) in the Airspace Manager, alongside the geofence. **Shipped**: backend read/write (points + RALLY_LIMIT_KM/RALLY_INCL_HOME), green R-markers in 2D+3D (draggable + popup), Rally panel section + layer toggle. Open: SITL verification. See `docs/active/GEOFENCE.md`_
...
- [ ] **Distinct alert tones for MAVLink system messages** — currently all STATUSTEXT/system messages share one tone; differentiate by severity (e.g. emergency/alert/critical vs warning/notice) so a fence breach or failsafe is audibly distinct from routine info.
