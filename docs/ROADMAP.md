...
- [x] **Waypoint parameter view** — per-WP parameter panel in the editor. Shipped as the catalog-driven detail panel (Lat/Lon/Alt + every command param as label · value · unit/enum) built with the ArduPilot/PX4 catalog work; INAV + ArduPilot/PX4. Footer detail no longer has a spurious inner scrollbar (relies on the pinned-footer / column-scroll PanelShell mechanics).
...
- [~] **Mission provenance + flown-vs-loaded validation** _(flag model + gating in `docs/archive/MISSION_TRACKING_AND_PROVENANCE.md`)_:
  ...
  - ~~**Validate flown vs. loaded**~~ — **CUT (2026-06-28):** the flown track never aligns precisely enough with the planned mission — too much positional uncertainty to flag divergence reliably.
  - [ ] **Verify mission-in-log support** — ArduPilot `.bin` embeds the mission (CMD messages); INAV blackbox only in later FW versions — confirm exact version + parse it on import. — _future (ArduPilot DataFlash only; not needed for release)._
- ~~**Disable/enable waypoint**~~ — **CUT (2026-06-19, not implementing):** no standard representation in `.waypoints`/mission-JSON → export round-trip incompatibility, risky model/codec complexity, low expected use. Design preserved in `docs/future/WaypointDisable.md`.
...
- [x] ArduPilot `.tlog` MAVLink log import — shipped via the unified raw importer (ADR-049): `flightlog_import_raw` → `decode_tlog`, split at arm/disarm; file-picker + drag&drop accept `.tlog` alongside `.rawmsp`
- [x] ArduPilot mission system (MAVLink WP protocol) — complete: `mavlink_proto/mission.rs` upload/download/clear microprotocol + `ardu_mission_download`/`ardu_mission_upload`, wired to the ArduPilot mission panel/layer; `.waypoints` (QGC WPL) file save/load/drop + INAV↔ArduPilot WP conversion. See `docs/archive/MISSION_MULTIAUTOPILOT_PLAN.md`
- [x] **PX4 mission planning** — rides the ArduPilot MAVLink mission pipeline; firmware-aware command catalog (verified PX4 subset, no `JUMP_TAG`/extra-loiter/relay/condition cmds), **no vehicle-type selector** (one PX4 interpreter for all airframes; class read from `MAV_TYPE`; connect-time soft-warning for VTOL-on-non-VTOL / unsupported cmds), and **firmware-aware home-slot** (PX4 has no home item 0, unlike ArduPilot — upload/download no longer inject/drop a placeholder). Untested on real PX4 hardware (verification not pursued — handled on user feedback). See `docs/archive/MISSION_MULTIAUTOPILOT_PLAN.md`
...
- [x] Phase D: DB recording for telemetry-mode flights — recorder wired (arm/disarm from FrSky MODES,
  fed the decoded telemetry). _Shipped; real-armed-flight verification deferred to user feedback._
- [x] `CrsfSource` — CRSF/ELRS telemetry frames (decode by frame/sub-type; INAV reworked these across versions)
- [x] `LtmSource` — LTM (Lightweight Telemetry) passive frame parser
- [x] MAVLink-passive decoder (reuse the MAVLink parser, TX disabled)
...
### Future Protocols
- ~~Multi-aircraft support~~ — **CUT (2026-06-28):** the multi-vehicle telemetry-monitoring concept was dropped.
...
### Map Overlays
- [x] **Airspace Manager** — a dedicated nav-rail panel (under Radar) + aeronautical-data subsystem over
  **OpenAIP**: **obstacles** (wind turbines/masts), **airspaces**, **RC/model airfields**, **airports**.
  ...
  Zoom-density management. **P1 + P2 shipped** (2D + 3D obstacles / airspaces / airport markers + persistence + density); P3 polish (extra alerts / legend / centre-fallback) is a future nicety. The static counterpart to the Radar
...
- [x] Survey / area planner — all six shapes complete:
  ...
  - ~~**Keep legs inside concave boundaries (don't cross gaps)**~~ — **CUT (2026-06-28):** on a concave polygon a single transfer leg can cross the concavity (outside the area), but routing it inside (boundary-walk / geodesic / convex split) is disproportionate geometry for ~2 s of operator time — and risks regressing the connected-fill order. After **Generate**, the operator can insert a mid-waypoint with one click and drag the transfer inside if it ever matters.
  - [ ] Load/save pattern templates
- [ ] OSD font/element preview
- [x] Safehome editor — _INAV Safe Home Manager: 8 slots + per-site FW autoland approach config, 2D + 3D map overlays, Save-to-FC. See `docs/archive/AUTOLAND_SAFEHOME.md`_
- [~] **Vehicle control — GCS command & guided steering** — command the vehicle directly from the GCS without a transmitter (à la Mission Planner / QGC), done better than MP's flat button grid. **V1 shipped (MAVLink, SITL-verified on ArduPilot Copter/Plane/QuadPlane; PX4 firmware-aware but untested on hardware):** `control` nav-rail panel (only when MAVLink-connected) — curated GCS-safe mode switching (stick-required modes hidden + **hard-locked via RC-presence detection**), Arm (slide-to-confirm) / Disarm + all actions via reusable **`HoldToConfirm`** (1 s), Takeoff / Land / RTL / **Abort Landing**, a **Guided toggle** + map-click "Fly Here" popup (`DO_REPOSITION`, vehicle-aware fields), active-flight adjust (Change Alt — Guided-only / Change Speed / Set Loiter Radius / Set Home / Set Heading — Copter-only), Mission start/restart/set-WP, **VTOL transition** (QuadPlane), async `COMMAND_ACK` feedback. Firmware divergence handled throughout (Mission Start, Land, loiter param, transition, PX4 packed custom_mode); robust QuadPlane `Q_ENABLE` detection; firmware-removed fixed-wing Set-Heading (ArduPlane guided-heading-slew bug). Direct mode buttons (not a dropdown). Recorded as ADR-052. _Deferred phases:_ **INAV guided** (WP #255 reposition + WP #0 live-home), **joystick/HID continuous RC** (the HID item below), **advanced outputs** (set-servo / gripper-payload / gimbal-ROI) on the right-hand `advanced` split panel. See `docs/archive/VEHICLE_CONTROL.md`.
- [~] HID controller input (gamepad/joystick) — joystick/HID phase of **Vehicle control** above. **INAV
  (Phases 1–4, pending on-hardware test):** native HID backend, channel-mapping profiles, safety locks,
  and the live MSP send pipeline (CH1–16 RAW_RC / CH17–32 AUX_RC, engage gate, seed-on-connect, deadman,
  link probe) — `docs/archive/MSP_RC_CONTROL.md`. **MAVLink — ArduPilot shipped (SITL-verified):**
  ...
  auto-load, `SYSID_MYGCS`/`COM_RC_IN_MODE` mismatch warnings — `docs/archive/MAVLINK_RC_CONTROL.md`.
...
- [~] Embedded video — _built: source router + webcam/USB-capture (`getUserMedia`, cross-platform), NavRail panel (live preview, 60 fps MJPEG fix), 2×1 dock widget, snap/drag floating window, double-click map⇄video swap, native Picture-in-Picture, persistence + auto-start. **v2:** RTSP/UDP network streams **shipped**; native `nokhwa` capture + snapshot/record still future; see `docs/archive/VideoFeature.md`_
- [ ] FW approach / autoland planner
- [x] Geozone editor — _INAV ≥8.0 FC geozones in the Airspace Manager. **P1 (display) + P2 (editor) + mission safety check shipped**: backend read/write+EEPROM, 2D+3D overlays (MWPTools type/action line+fill scheme, blue incl / amber excl), 5th layer toggle + zone list, map-first editing (drag handles + WP-style popup + add/delete) with sanity checks, and an altitude-aware mission↔geozone safety check (warns on NFZ cross / inclusion leave / launch-in-NFZ, red legs in 2D+3D, hints only). **P3 done**: in-flight breach toast — since INAV doesn't expose its geozone-avoidance mode (`NAV_SEND_TO`) over MSP, the GCS detects an actual breach geometrically (3D) from the live position and toasts on onset (NFZ / left inclusion). Global `geozone_*` behaviour settings are **not planned** (FC zone definitions only). See `docs/archive/GEOZONES.md`_
- [x] Geofence editor — _ArduPilot/PX4 FC geofence in the Airspace Manager (third safety subsystem, MAVLink `MAV_MISSION_TYPE_FENCE`). **Full editor shipped**: backend read/write (params + geometry), 2D+3D overlays (blue incl / amber excl), 6th layer toggle + Fence section, map-first editing (drag handles + WP-style popup + add/delete), global params with friendly labels/units/bounds + a vehicle-aware breach-action dropdown. ArduPilot validated in SITL; PX4 untested — handled on user feedback. (In-flight breach: the FC's MAVLink STATUSTEXT is the primary cue → system messages; a geometric fence-breach toast fallback also ships via the shared breach monitor.) See `docs/archive/GEOFENCE.md`_
- [x] Rally points editor — _ArduPilot/PX4 RTL divert points (`MAV_MISSION_TYPE_RALLY`) in the Airspace Manager, alongside the geofence. **Shipped**: backend read/write (points + RALLY_LIMIT_KM/RALLY_INCL_HOME), green R-markers in 2D+3D (draggable + popup), Rally panel section + layer toggle. ArduPilot validated; PX4 on user feedback. See `docs/archive/GEOFENCE.md`_
...
Subsystem detailed in `docs/archive/RADAR_TRACKING_*`, `RADAR_ALERTS.md`, `docs/archive/RADAR_FORMATION_FLIGHT.md` (ADR-033/035/036). Largely shipped (ADS-B online + receivers + via-MSP, conflict alerts, FormationFlight). Open items:
...
- [x] **Conflict-alert tuning (C3)** — shipped: the 5 thresholds (proximity radius + vertical sep; CPA radius/height/look-ahead) are user-editable under the 2 stage toggles, persisted, fed as overrides into the evaluator. Pre-recorded callout audio for engines without TTS (Linux WebKitGTK) stays a later add-on.
- [x] **Distinct alert tones for MAVLink system messages** — currently all STATUSTEXT/system messages share one tone; differentiate by severity (e.g. emergency/alert/critical vs warning/notice) so a fence breach or failsafe is audibly distinct from routine info. **Done** — severity-tiered tones in statusText.ts (info / warning / error).
...
- [x] **Terrain analysis with MAVLink (ArduPilot/PX4) missions** — fixed: the Terrain Analyzer uses an autopilot-aware mission source (Ardu frame→AMSL/AGL/rel, FC home as launch ref); RF origin falls back to a Takeoff waypoint + hint. Editing (correction/Add-WP) stays INAV-only for now.
...
- [x] **Altitude curtain + ground shadow** under the 3D track (vertical wall to ground + terrain-draped shadow; flight-mode coloured; Settings→Map toggle; in replay built progressively behind the UAV, chunked for scale + reverse-scrub debounce). See `docs/archive/Map3DRework.md`
...
*Last updated: 2026-06-19 (Vehicle control — GCS command & guided steering V1 shipped, MAVLink ArduPilot/PX4; PX4 mission planning + offline takeoff WP shipped)*
