...
- [~] Geozone editor — _INAV ≥8.0 FC geozones in the Airspace Manager. **P1 (read-only display) shipped**: backend read + handshake download, 2D + 3D overlays with the MWPTools type/action line+fill scheme (blue inclusive / amber exclusive), 5th layer toggle + zone list. **P2** (interactive 2D editor + Save-to-FC + sanity checks), then **P3** (breach alerts). See `docs/active/GEOZONES.md`_
