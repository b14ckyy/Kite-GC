...
pub(crate) fn read_setting(handle: &MspRequester, name: &str) -> Result<Vec<u8>, String> {
...
pub(crate) fn set_setting(handle: &MspRequester, name: &str, value: &[u8]) -> Result<(), String> {
    let mut payload = name.as_bytes().to_vec();
    payload.push(0);
...
/// `Ok(None)` if the setting is absent (older/limited firmware: empty reply); a failed request (timeout,
/// link gone) stays `Err` — a lossy link (MSP over MAVLink) must never turn into a silent default.
pub(crate) fn try_read_uint_setting(handle: &MspRequester, name: &str) -> Result<Option<u64>, String> {
    let b = read_setting(handle, name)?;
    Ok(match b.len() {
        1 => Some(b[0] as u64),
...
pub(crate) fn set_uint_setting(handle: &MspRequester, name: &str, value: u64) -> Result<(), String> {
